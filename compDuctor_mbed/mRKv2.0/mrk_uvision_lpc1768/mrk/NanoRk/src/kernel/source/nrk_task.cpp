...
void nrk_add_to_readyQ (int8_t task_ID)
{
    ...
    if (_head_node != NULL)
    {
        ...
        while (NextNode != NULL)
        {
					   //nrk_task_TCB[task_ID].dead_line = nrk_task_TCB[task_ID].next_period - nrk_task_TCB[task_ID].period +  nrk_task_TCB[task_ID].r_deadline;
            ...
            if (nrk_task_TCB[NextNode->task_ID].task_prio <
                ...
                break;
					...
					/*
                      if((nrk_task_TCB[task_ID].next_period - nrk_task_TCB[task_ID].period +  nrk_task_TCB[task_ID].r_deadline < _nrk_prev_timer_val)&&
											(nrk_task_TCB[task_ID].next_period - nrk_task_TCB[task_ID].period +  nrk_task_TCB[task_ID].r_deadline > 0))
		                {
                      printf("Task_%d missed its deadline!\r\n", task_ID);
								
							        printf("Task ID: %d, nrk_prev_timer_val: %d, absolute deadline: %d, next period: %u\r\n", task_ID, _nrk_prev_timer_val,
							       (nrk_task_TCB[task_ID].next_period - nrk_task_TCB[task_ID].period +  nrk_task_TCB[task_ID].r_deadline),nrk_task_TCB[task_ID].next_period);
										...
										{
                    printf("Too many missed deadlines, I'm done!");
									  nrk_terminate_task();
										nrk_halt();
                    }
					  ...
					  //printf("Arrived!\r\n");
					  if ((nrk_task_TCB[NextNode->task_ID].next_period - nrk_task_TCB[NextNode->task_ID].period + nrk_task_TCB[NextNode->task_ID].r_deadline ) >
                    (nrk_task_TCB[task_ID].next_period - nrk_task_TCB[task_ID].period +  nrk_task_TCB[task_ID].r_deadline)||NextNode->task_ID == NRK_IDLE_TASK_ID)
            break;
            */
